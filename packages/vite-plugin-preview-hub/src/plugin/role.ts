...
import type { PreviewRole } from "./options";

/**
 * Resolve the effective role. An explicit `hub`/`instance` always wins. Otherwise `auto` is a
 * heuristic (D1): instance when the launch script has exported preview env vars, or when running
 * inside a linked git worktree; hub otherwise.
 *
 * Pure over its inputs (env + cwd are passed in) so it is testable without mutating the process.
 */
export function resolveRole(
  explicit: PreviewRole,
  env: NodeJS.ProcessEnv,
  cwd: string,
): "hub" | "instance" {
  if (explicit === "hub" || explicit === "instance") {
    return explicit;
  }

  // The launch script (D3/D4) sets these before spawning an instance's dev command.
  if (isNonEmpty(env["PREVIEW_NAME"]) || isNonEmpty(env["PREVIEW_HUB_BASE"])) {
    return "instance";
  }
