...
/**
 * Handle the hub's control endpoints under {@link CONTROL_PREFIX} (D2):
 *
 * - `POST /register` — register or heartbeat (idempotent upsert)
 * - `DELETE /register` — graceful deregister
 * - `GET /health` — hub liveness probe for instance startup
 * - `GET /list` — registry snapshot as JSON
 * - `GET /config` — hub config (mount path) for the DevTools client
 *
...
export async function handleControlRequest(
  ...
  if (sub === "/config" && method === "GET") {
    sendJson(res, 200, { mountPath: deps.mountPath });
    return true;
  }
