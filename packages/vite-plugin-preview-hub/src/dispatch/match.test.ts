...
import { isIndexPath, isPortInRange, matchPreviewName } from "./match";

describe("matchPreviewName", () => {
  it("extracts the name from a nested path", () => {
    expect(matchPreviewName("/preview/foo/bar", "/preview")).toBe("foo");
  });

  it("extracts the name when there is no trailing path", () => {
    expect(matchPreviewName("/preview/foo", "/preview")).toBe("foo");
  });

  it("ignores the query string", () => {
    expect(matchPreviewName("/preview/foo/?token=abc", "/preview")).toBe("foo");
  });

  it("returns null for the index path", () => {
    expect(matchPreviewName("/preview/", "/preview")).toBeNull();
  });

  it("returns null for an unrelated path", () => {
    expect(matchPreviewName("/other/foo", "/preview")).toBeNull();
  });
