...
/**
 * The slice of the resolved plugin options the gateway role needs. Declared here (rather than
 * importing the plugin's `ResolvedGatewayOptions`) so the dependency runs plugin -> gateway, never
 * the reverse.
 */
...
export function setupGateway(
  ...
  server.middlewares.use((req, res, next) => {
    void (async () => {
      ...
      const name = matchInstanceName(url, mountPath);
      if (name === null) {
        next();
        return;
      }
  ...
  server.httpServer?.on("upgrade", (req, socket, head) => {
    const name = matchInstanceName(req.url ?? "", mountPath);
    if (name === null) {
...
function buildGatewayInfo(server: ViteDevServer): GatewayInfo | null {
  ...
  try {
    const identity = resolveIdentity("rootDir", server.config.root);
    name = identity.name;
